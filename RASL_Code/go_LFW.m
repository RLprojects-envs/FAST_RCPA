...
dset=dir(strcat(imagePath,'\','0*'));
for index=1:length(dset)
mkdir(strcat(imagePath,'\',dset(index).name,'\',dset(index).name,'result-RASL'));
userName = dset(index).name ;
% one of 
...
%% plot the results
for c=1:size(A,2);
    temp=(mat2gray(reshape(A(:,c), raslpara.canonicalImageSize)));
    imwrite(temp,strcat(imagePath,'\',dset(index).name,'\',dset(index).name,'result-RASL','\',num2str(c),'.jpg'))
end

meanimage=mat2gray((reshape(sum(A,2), raslpara.canonicalImageSize)));
imwrite(meanimage,strcat('rasl_prima','\',dset(index).name,'.jpg'));
%imwrite(meanimage,strcat(imagePath,'\',dset(index).name,'\',dset(index).name,'result-RASL','\',dset(index).name,'mean.jpg'));
end
